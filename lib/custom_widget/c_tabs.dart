///描述:自定义tabs
///功能介绍:提供修改高度等其他属性（从tabs源码拷贝而来，进行自定义数据更改）
///创建者:翁益亨
///创建日期:2022/7/5 9:50
...
const double _kTabLabelPadding = 0;//tab之间的间距设定（左右两边间距设置）

///贴边滚动数据处理
const double kTabLabelMiddlePadding = 14;//tab之间的间距设定（左右两边间距设置）
...
///  重写：这里需要额外处理水平可滑动边距信息，最左边和最右边间距20，tab之间的距离为28.(前提isScrollable设置为false的情况，适配数据)
...
class _CTabStyle extends AnimatedWidget {
  const _CTabStyle({
    Key? key,
...
class _CTabLabelBarRenderer extends RenderFlex {
  _CTabLabelBarRenderer({
...
class _CTabLabelBar extends Flex {
  _CTabLabelBar({
    Key? key,
  ...
  @override
  void updateRenderObject(BuildContext context, _CTabLabelBarRenderer renderObject) {
...
class _CIndicatorPainter extends CustomPainter {
  _CIndicatorPainter({
    required this.controller,
    required this.indicator,
    required this.indicatorSize,
    required this.tabKeys,
    required _CIndicatorPainter? old,
  ...
  @override
  bool shouldRepaint(_CIndicatorPainter old) {
    return _needsPaint
...
class _CChangeAnimation extends Animation<double> with AnimationWithParentMixin<double> {
  _CChangeAnimation(this.controller);
...
class _CDragAnimation extends Animation<double> with AnimationWithParentMixin<double> {
  _CDragAnimation(this.controller, this.index);
...
class _CTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _CTabBarScrollPosition({
...
class _CTabBarScrollController extends ScrollController {
  _CTabBarScrollController(this.tabBar);

  final _CTabBarState tabBar;

  @override
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition? oldPosition) {
    return _CTabBarScrollPosition(
      physics: physics,
      context: context,
      oldPosition: oldPosition,
      tabBar: tabBar,
...
class _CTabBarState extends State<CTabBar> {
  ...
  _CIndicatorPainter? _indicatorPainter;
  int? _currentIndex;
  ...
  void _initIndicatorPainter() {
    _indicatorPainter = !_controllerIsValid ? null : _CIndicatorPainter(
      controller: _controller!,
      indicator: _indicator,
      //这里TabBarTheme.of(context).indicatorSize获取的还是tabs.dart文件中的indicatorSize，所以这里我们也要使用相同的内容
  ...
  Widget build(BuildContext context) {
      ...
      if (widget.tabs[index] is PreferredSizeWidget) {
        ...
        if (widget.tabHasTextAndIcon && tab.preferredSize.height == _kTabHeight) {
          if (widget.labelPadding != null || tabBarTheme.labelPadding != null) {
            ...
            //修改处，修改左右边距，设置为_kTabLabelPadding
            adjustedPadding = const EdgeInsets.symmetric(vertical: verticalAdjustment, horizontal: _kTabLabelPadding);//horizontal: 16.0
          }
        }
      }

      //修改处，使内部数据占满整个容器，全局constants的kTabLabelPadding中设置的是左右边距16，所以这里要取消边距内容，使用自定义的方式进行处理

      return Center(
        heightFactor: 1.0,
        child: Padding(
          //kTabLabelPadding 替换为本地的 EdgeInsets.symmetric(horizontal: _kTabLabelPadding)
          padding: adjustedPadding ?? widget.labelPadding ?? tabBarTheme.labelPadding ?? EdgeInsets.symmetric(horizontal: widget.isScrollable?widget.middlePadding:_kTabLabelPadding),
          child: KeyedSubtree(
            key: _tabKeys[index],
    ...
    if (_controller != null) {
      ...
      if (_controller!.indexIsChanging) {
        ...
        final Animation<double> animation = _CChangeAnimation(_controller!);
        ...
        final Animation<double> centerAnimation = _CDragAnimation(_controller!, tabIndex);
        wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], true, centerAnimation);
        if (_currentIndex! > 0) {
          final int tabIndex = _currentIndex! - 1;
          final Animation<double> previousAnimation = ReverseAnimation(_CDragAnimation(_controller!, tabIndex));
          wrappedTabs[tabIndex] = _buildStyledTab(wrappedTabs[tabIndex], false, previousAnimation);
        }
        if (_currentIndex! < widget.tabs.length - 1) {
          final int tabIndex = _currentIndex! + 1;
          final Animation<double> nextAnimation = ReverseAnimation(_CDragAnimation(_controller!, tabIndex));
    ...
    if (widget.isScrollable) {
      _scrollController ??= _CTabBarScrollController(this);
...
class CTabBarView extends StatefulWidget {
  ...
  @override
  State<TabBarView> createState() => _CTabBarViewState();
}

class _CTabBarViewState extends State<TabBarView> {
  TabController? _controller;
...
class CTabPageSelectorIndicator extends StatelessWidget {
  /// Creates an indicator used by [TabPageSelector].
  ///
  /// The [backgroundColor], [borderColor], and [size] parameters must not be null.
  const CTabPageSelectorIndicator({
    Key? key,
...
/// If a [TabController] is not provided, then there must be a
/// [DefaultTabController] ancestor.
class CTabPageSelector extends StatelessWidget {
  /// Creates a compact widget that indicates which tab has been selected.
  const CTabPageSelector({
    Key? key,
    this.controller,
